...
def test_get_input_amount(registry_swap, swap, underlying_coins, underlying_decimals, send, recv, st_amount):
...
@given(st_amount=strategy("decimal", min_value="0.5", max_value=1000, places=2))
@settings(max_examples=50, phases=[Phase.generate])
def test_get_input_amount_wrapped(registry_swap, swap, wrapped_coins, wrapped_decimals, send, recv, st_amount):
